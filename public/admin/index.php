...
$stmt = $pdo->query("SELECT COUNT(*) AS total FROM voters");
$totalVoters = $stmt->fetch()['total'];
$stmt = $pdo->query("SELECT COUNT(*) AS total FROM voters WHERE has_voted='1'");
$hasVoted = $stmt->fetch()['total'];
$stmt = $pdo->query("SELECT COUNT(*) AS total FROM voters WHERE has_voted='0'");
$notVoted = $stmt->fetch()['total'];
$stmt = $pdo->query("SELECT COUNT(*) AS total FROM candidates WHERE is_active='1'");
  ...
  <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-72 -translate-x-full lg:static lg:translate-x-0 lg:z-auto border-r <?php echo $theme === 'dark' ? 'border-slate-800 bg-slate-900/80' : 'border-slate-200 bg-white/90'; ?> p-6 flex flex-col justify-between transition-transform duration-300">
    ...
    <main class="flex-1 p-8 pt-16 lg:pt-8">
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm uppercase tracking-[0.3em] text-amber-400">Dashboard Admin</p>
          <h1 class="text-3xl font-semibold">Ringkasan Pemilihan</h1>
        </div>
        <div class="rounded-2xl border <?php echo $theme === 'dark' ? 'border-slate-700' : 'border-slate-300 bg-white'; ?> px-4 py-3 text-sm">
          Status:
          <?php
            $statusLabel = ['draft' => 'DRAFT', 'open' => 'DIBUKA', 'closed' => 'DITUTUP'];
            $statusColor = ['draft' => 'text-amber-400', 'open' => 'text-emerald-400', 'closed' => 'text-red-400'];
            $vs = $settings['voting_status'] ?? 'draft';
          ?>
          <span class="font-semibold <?php echo $statusColor[$vs] ?? 'text-slate-400'; ?>">
            <?php echo $statusLabel[$vs] ?? strtoupper($vs); ?>
          </span>
        </div>
      </div>
      <div class="mt-8 grid gap-4 md:grid-cols-4">
        <div class="rounded-3xl border <?php echo $theme === 'dark' ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-white'; ?> p-6"><p class="text-sm <?php echo $theme === 'dark' ? 'text-slate-400' : 'text-slate-600'; ?>">Total Pemilih</p><p class="mt-2 text-3xl font-semibold"><?php echo (int)$totalVoters; ?></p></div>
        <div class="rounded-3xl border <?php echo $theme === 'dark' ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-white'; ?> p-6"><p class="text-sm <?php echo $theme === 'dark' ? 'text-slate-400' : 'text-slate-600'; ?>">Sudah Memilih</p><p class="mt-2 text-3xl font-semibold"><?php echo (int)$hasVoted; ?></p></div>
        <div class="rounded-3xl border <?php echo $theme === 'dark' ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-white'; ?> p-6"><p class="text-sm <?php echo $theme === 'dark' ? 'text-slate-400' : 'text-slate-600'; ?>">Belum Memilih</p><p class="mt-2 text-3xl font-semibold"><?php echo (int)$notVoted; ?></p></div>
        <div class="rounded-3xl border <?php echo $theme === 'dark' ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-white'; ?> p-6"><p class="text-sm <?php echo $theme === 'dark' ? 'text-slate-400' : 'text-slate-600'; ?>">Total Kandidat</p><p class="mt-2 text-3xl font-semibold"><?php echo (int)$totalCandidates; ?></p></div>
      </div>
      <div class="mt-8 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div class="rounded-3xl border border-slate-800 bg-slate-900 p-6">
          <h2 class="text-xl font-semibold">Grafik Partisipasi</h2>
          <canvas id="participationChart" class="mt-6 h-72"></canvas>
        </div>
        <div class="rounded-3xl border border-slate-800 bg-slate-900 p-6">
          <h2 class="text-xl font-semibold">Informasi</h2>
          <div class="mt-6 space-y-3 text-sm <?php echo $theme === 'dark' ? 'text-slate-300' : 'text-slate-700'; ?>">
            <div class="rounded-2xl <?php echo $theme === 'dark' ? 'bg-slate-800' : 'bg-slate-100'; ?> p-4">Sekolah: <?php echo htmlspecialchars($settings['school_name']); ?></div>
            <div class="rounded-2xl <?php echo $theme === 'dark' ? 'bg-slate-800' : 'bg-slate-100'; ?> p-4">Tema: <?php echo htmlspecialchars($settings['theme']); ?></div>
            <div class="rounded-2xl <?php echo $theme === 'dark' ? 'bg-slate-800' : 'bg-slate-100'; ?> p-4">Welcome: <?php echo htmlspecialchars($settings['welcome_text'] ?: '-'); ?></div>
          </div>
        </div>
      </div>
    </main>
    ...
    if (ctx) {
      new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{ label: 'Jumlah', data: [<?php echo (int)$hasVoted; ?>, <?php echo (int)$notVoted; ?>], backgroundColor: ['#2563eb', '#f59e0b'] }]
        },
